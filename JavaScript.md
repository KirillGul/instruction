...
1. или подключён из внешнего файла `<script scr="путь/до/файла"></script>`
...
# Переменные
1. Переменная должна состоять из [a-zA-Z0-9$_] латинских букв, чисел, символов $ и знаков подчеркивания.
 ...
 * \` ... \` -  специальный тип кавычек - обрытные кавычки (косые), в косых кавычках можно выполнять вставку переменных. Для этого имя переменной нужно написать в конструкции ${}, также косые кавчки поддерживают многострочность
...
* `break` позволяет досрочно завершать цикл и перейти к след. строке после цикла
* `continue`, запускающая новую итерацию цикла.
...
После закрывающе фигурной скобки "}" обязательно ставится точка с запятой ";".
