...
Есть две важные особенности имени `func`(или другое) (это название своего рода `this` для функции), ради которого оно даётся:
...
# Прототипы, наследование в объектах
...
### Методы не «свободны»
Единственное место в языке, где используется `[[HomeObject]]` – это `super`. Поэтому если метод не использует `super`, то мы все ещё можем считать его свободным и копировать между объектами.  
...
1. Внешний интерфейс – методы и свойства, доступные снаружи класса.
...
1. Приватные: доступны только внутри класса. Они для внутреннего интерфейса.
...
* Если мы унаследуем, то мы не получим прямого доступа к защищенным свойствам и методам. Мы будем вынуждены полагаться на геттер/сеттер.
...
## Примеси
<u>Примесь</u> – это класс, методы которого предназначены для использования в других классах, причём без наследования от примеси.  
...
* `executor` - функция, переданная в конструкцию `new Promise`, называется исполнитель (executor). Когда `Promise` создаётся, она запускается автоматически. Она должна содержать «создающий» код, который когда-нибудь создаст результат.
...
Когда он получает результат, сейчас или позже – не важно, он должен вызвать один из этих колбэков:
	...
	* `reject(error)` — если произошла ошибка, `error` – объект ошибки. Вызывайте reject с объектом `Error`.
* возврат значения объект `Promise` готовый для исполнения, с помощью `then`, `catch`, `finally`. 
...
Успешный, и отклонённый будем называть «завершённым», в отличие от изначального промиса «в ожидании». Может быть что-то одно: либо результат, либо ошибка.  
...
1. Обработчик finally «пропускает» результат или ошибку дальше, к последующим обработчикам.  
...
1. Обработчик finally также не должен ничего возвращать. Если это так, то возвращаемое значение молча игнорируется.  
...
## Fetch
<u>Fetch</u> - метод чтобы делать сетевые запросы и получать информацию с сервера.  
«AJAX» (аббревиатура от Asynchronous JavaScript And XML)

`let promise = fetch(url, [options])` - этот код запрашивает по сети url и возвращает промис.
...
1. Во-первых, `promise` выполняется с объектом встроенного класса `Response` в качестве результата, как только сервер пришлёт заголовки ответа.  
	...
	* `ok` – логическое значение: будет true, если код HTTP-статуса в диапазоне 200-299.
	...
	```
1. Во-вторых, для получения тела ответа нам нужно использовать дополнительный вызов метода (которые тоже возвращают промис).  
...
### Promise.all
---
* Запустить множество промисов параллельно и дождаться, пока все они выполнятся и вернуть новый промис.  
* Новый промис завершится, когда завершится весь переданный список промисов, и его результатом будет массив их результатов.  
* Если любой из промисов завершится с ошибкой, то промис, возвращённый Promise.all, немедленно завершается с этой ошибкой. Если один промис завершается с ошибкой, то весь Promise.all завершается с ней, полностью забывая про остальные промисы в списке. Их результаты игнорируются.
* `Promise.all(iterable)` разрешает передавать не-промисы в `iterable` (перебираемом объекте), если любой из этих объектов не является промисом, он передаётся в итоговый массив «как есть».
...
]).then(alert); // 1 Быстрее всех выполнился первый промис, он и дал результат. После этого остальные промисы игнорируются.
```
...
* `Promise.resolve(value)` - создаёт успешно выполненный промис с результатом value, то же самое что и `let promise = new Promise(resolve => resolve(value));`
...
## Async/await
### async - aсинхронные функции
---
У слова `async` один простой смысл: эта функция всегда возвращает промис. Значения других типов оборачиваются в завершившийся успешно промис автоматически.

1. Обязывает её всегда возвращать промис.
1. Позволяет использовать await в теле этой функции.
```javascript
async function f() {
  return 1;
}
...
1. Иначе вернётся результат промиса.
...
<u>Генераторы</u> - могут порождать (yield) множество значений одно за другим, по мере необходимости.

`function*` - специальное обозначение «функция-генератор». Функции-генераторы ведут себя не так, как обычные. Когда такая функция вызвана, она не выполняет свой код. Вместо этого она возвращает специальный объект, так называемый «генератор», для управления её выполнением.
...
Обратите внимание: пример выше выводит значение 1, затем 2. Значение 3 выведено не будет! Т.к. на `return` перебор останавливается. Поэтому, если мы хотим, чтобы были все значения при переборе через `for..of`, то надо возвращать их через `yield`.
...
1. `next()` должен возвращать промис.
1. Чтобы перебрать такой объект, используется цикл `for await (let item of iterable)`.

> Оператор расширения ... не работает асинхронно, т.е. `alert( [...range] ); // Ошибка, нет Symbol.iterator`
...
# Массивы
* массивы это объекты, у которых прототип `Array`
	```javascript
	let arr = [];
	```
...
* длина массива находится в свойстве `length`
	```javascript
	let arr = ['a', 'b', 'c'];
...
> Всё это приведет к плохо оптимизированному массиву.
...
## Методы примитивов
---
<u>Методы примитовов</u> - это создание специального «объекта-обёртки», который предоставляет нужную функциональность объекта, а после удаляется.
...
* `строка.repeat(строка, сколько раз повторить); :String` - повторяет строку несколько раз.  
...
* `строка.fromCodePoint(код); :String` - создаёт символ по его коду  
* `строка.localeCompare(str2): Integer(-1,+1,0)` - возвращает число, которое показывает, какая строка больше в соответствии с правилами языка:
...
* `string.split([разделитель], [максимальное количество элементов]); :Array` - разбивает строку в массив.  
...
* `array.forEach(function(элемент, индекс, массив) { /* код */ }); :void` - позволяет последовательно перебрать все элементы массива и запустить для них функцию. Метод в параметре получает функцию, которая выполнится для каждого элемента массива. В эту функцию можно передавать три параметра. Если эти параметры есть (они не обязательны), то в первый автоматически попадет элемент массива, во второй попадет его номер в массиве (индекс), а в третий - сам массив. Если что-то возвращает, то результат отбрасывается.

* `array.map(function(элемент, индекс, массив) { /* код */ return измененный элемент; }); :void+Array` - позволяет применить заданную функцию для каждого элемента массива. При этом метод не <u>изменяет исходный массив</u>, а возвращает измененный, в отличии от `array.forEach`.  
...
* `array.indexOf(item, from); :Integer|-1` - ищет item начиная с индекса from и возвращает номер индекса, на котором был найден искомый элемент, в противном случае -1.

* `array.lastIndexOf(item, from); :Integer|-1` - метод arr.lastIndexOf похож на indexOf, но ищет справа налево.

* `array.includes(item, from); :Boolean` - ищет item начиная с индекса from и возвращает true, если поиск успешен.
...
Чтобы использовать наш собственный порядок сортировки, нам нужно предоставить функцию в качестве аргумента `array.sort(func)`.
	...
	* для строк используйте функция `localeCompare`  
...
* `Объект.getTime()` - возвращает timestamp, учтите, что, в отличие от некоторых других систем, в JavaScript таймстамп в миллисекундах, а не в секундах (1 сек. = 1000 мс.).
...
---
* `Объект.setFullYear(year, [month], [date])`
* `Объект.setMonth(month, [date])`
* `Объект.setDate(date)`
* `Объект.setHours(hour, [min], [sec], [ms])`
* `Объект.setMinutes(min, [sec], [ms])`
* `Объект.setSeconds(sec, [ms])`
* `Объект.setMilliseconds(ms)`
* `Объект.setTime(milliseconds)` - (устанавливает дату в виде целого количества миллисекунд, прошедших с 01.01.1970 UTC)
...
---
`Date.parse(str)` - метод считывает дату из строки в timestamp.  
...
## JSON.parse
`JSON.parse(str, [reviver]);` - десериализовать строку в объект
...
* `table.rows` – коллекция строк `<tr>` таблицы(или `tBodies` и др.)
...
---
`elem.insertAdjacentHTML(where, html)` - вставляеются строка html с обработкой тегов.  
...
> Если нам нужно переместить элемент в другое место – нет необходимости удалять его со старого. Все методы вставки (`append`, `prepend` и др.) автоматически удаляют узлы со старых мест.
...
---
> scrollbar отнимается от width элемента
...
* `offsetParent` – ближайший CSS-позиционированный родитель или ближайший td, th, table, body.
	...
	1. Для элементов `<body>` и `<html>`.
	1. Для элементов с `position:fixed`.
* `offsetLeft/offsetTop` – позиция в пикселях верхнего левого угла относительно `offsetParent` (расстояние от родителя).
	...
	1. или `<td>`, `<th>`, `<table>`,
	1. или `<body>`.
* `offsetWidth/offsetHeight` – «внешняя» ширина/высота элемента, с рамкой(`border`) и отступами(`padding`).
	...
	```
* `clientLeft/clientTop` – расстояние от верхнего левого внешнего угла до внутренного. Для операционных систем с ориентацией слева-направо эти свойства равны ширинам левой/верхней рамки. Если язык ОС таков, что ориентация справа налево(арабский), так что вертикальная полоса прокрутки находится не справа, а слева, то `clientLeft` включает в своё значение её ширину. (ширина рамки с прокрутой, если она есть).
* `clientWidth/clientHeight` – ширина/высота содержимого вместе с внутренними отступами `padding`, но без полосы прокрутки(`scrollbar`) и без рамки(`border`).  
	Иными словами, они возвращают высоту/ширину видимой части документа, доступной для содержимого.
...
Важно: Все свойства доступны только для чтения, кроме `scrollLeft/scrollTop`, изменение которых заставляет браузер прокручивать элемент.

## Ширина/высота окна
### Получение ширины/высота окна
---
* Ширина/высота <u>видимой части документа</u> (ширина/высота области содержимого) для `html` работает на видимой области в отличии от элемента, которая показывает ширину и высоту документа: 
	```javascript
	document.documentElement.clientWidth; //ширина
	document.documentElement.clientHeight; //высота
	```

	> Не `window.innerWidth/Height`, т.к. включает в себя полосу прокрутки.
* Ширина/высота <u>всего документа со всей прокручиваемой областью страницы</u> (лучше вычислять по максимальному размеру из...):
	```javascript
...
1. <u>Использование атрибута HTML</u>
	1. Обработчик прямо в HTML
	```html
	<input value="Нажми меня" onclick="alert('Клик!')" type="button">
	```
	
	2. Обработчик в виде функции
	```javascript
...
* `event.eventPhase` - содержащее номер фазы, на которой событие было поймано.
* `event.target` - при наступлении события – самый глубоко вложенный элемент, на котором оно произошло, помечается как «целевой».
...
## Объект-обработчик: с функцией handleEvent
---
...
<u>Всплытие</u> - принцип всплытия очень простой. Когда на элементе происходит событие, обработчики сначала срабатывают на нём, потом на его родителе, затем выше и так далее, вверх по цепочке предков.

> Почти все события всплывают. Ключевое слово в этой фразе – «почти». Например, событие `focus` не всплывает. В дальнейшем мы увидим и другие примеры. Однако, стоит понимать, что это скорее исключение, чем правило, всё-таки большинство событий всплывают.
...
> Возврат false из обработчика – это исключение. Обычно значение, которое возвращает обработчик события, игнорируется. Единственное исключение – это return false из обработчика, назначенного через on<событие>. В других случаях return не нужен, он никак не обрабатывается.
...
* `options` – объект с тремя необязательными свойствами:
	...
	* `cancelable: true/false` – если `true`, тогда можно отменить действие по умолчанию, если мы хотим, чтобы `event.preventDefault()` работал.
...
<u>Метод `dispatchEvent`</u> - после того, как объект события создан, мы должны запустить его на элементе, вызвав метод `elem.dispatchEvent(event)`.
...
> event.isTrusted - принимает значение true для событий, порождаемых реальными действиями пользователя, и false для генерируемых кодом.
...
Изображения `<img src="...">`, `<style src="...">`, `<scrip src="...">` и другие ресурсы предоставляют события `load` и `error` для отслеживания загрузки:
...
Сборка мусора: объекты `MutationObserver` используют внутри себя так называемые «слабые ссылки» на узлы, за которыми смотрят. Так что если узел удалён из DOM и больше не достижим, то он будет удалён из памяти вне зависимости от наличия наблюдателя.
