...
- Класс - это чертеж, по которым на заводе делаются автомобили. Доп.: название класса CamelCase.
- Объект же - это сама машина, сделанная по этим чертежам. Доп.: название объекта с маленькой буквы.
...
Инкапсуляциия - все лишнее не должно быть доступно извне (чтобы вспомогательные методы нельзя было использовать вне нашего класса, в PHP используется "private").
...
Наследование - это наследование свойств и методов от родительского класса.
...
---------------------
class - создание класса (class Car {свойства и методы})
...
$this - для того, чтобы обратиться к свойству класса внутри метода этого класса, вместо имени объекта следует писать специальную переменную $this
new - создание объекта c определенно класса ($mycar = new Car;)
-> - обращение к свойству или методу объекта ($myCar->go();)
...
3) protected - чтобы некоторые методы или свойства родителя наследовались потомками, но при этом для всего остального мира вели себя как приватные.
Доп.: 
...
Контроль типов - для этого перед именем переменной параметра следует написать имя ожидаемого класса.
---------------
...
Константы классов:
------------------
const - чтобы создать константу, ее нужно объявить через ключевое слово const, затем написать имя константы без доллара и обязательно сразу же задать ее значение.
	Константы класса задаются один раз для всего класса, а не отдельно для каждого объекта этого класса.
	const CONSTANT = 'test'; - создание
	echo Test::CONSTANT; - обращение напрямую
...
- Функция spl_autoload_register пишется только в начале того файла, который вызывается через адресную строку браузера. 
	То есть, если какой-то класс вызывает внутри себя другой класс или наследует от кого-то, а сам класс вызывается в другом файле, 
...
3) Скачиваем и устанавливаем желательно"в ручную" Composer (в папку с:\php) (https://getcomposer.org/) : composer -V
	...
	rundll32.exe keymgr.dll,KRShowKeyMgr - проверка сохраненных паролей на Windows
